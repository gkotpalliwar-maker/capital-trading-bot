...
echo "=== v2.3.4 Installer ($BD) ==="
BK="$BD/backups/pre_v234_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BK"
...
for f in signal_scorer.py signal_scorer_commands.py trade_manager.py trade_manager_commands.py market_hours.py recall_commands.py pnl_commands.py; do
...
sed -i "s/v2\.[0-9]\.[0-9]*/v2.3.4/g" bot/scanner.py 2>/dev/null || true
echo ""
echo "Done! systemctl restart trading-bot"
echo "New: /fixpnl - Re-fetch P&L from API for suspect trades"
