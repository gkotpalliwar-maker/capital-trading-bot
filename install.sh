...
echo "=== v2.3.3 Installer ($BD) ==="
BK="$BD/backups/pre_v233_$(date +%Y%m%d_%H%M%S)"
mkdir -p "$BK"
...
for f in signal_scorer.py signal_scorer_commands.py trade_manager.py trade_manager_commands.py market_hours.py recall_commands.py; do
...
echo "Running v2.3.3 patcher (market hours, conflict, regime, recall)..."
python3 "$SD/patches/v2.3.3_patcher.py"
sed -i "s/v2\.[0-9]\.[0-9]*/v2.3.3/g" bot/scanner.py 2>/dev/null || true
echo ""
echo "Done! systemctl restart trading-bot"
echo "New: /recall [hours|Xd] - Recall past signals with live validation"
